...
module.exports = {
    'value formatters': function(done) {
    ...
    'test wrapping long lines': function(done) {
        var vevent = new icalendar.VEvent('testuid@daybilling.com');
    ...
    'rrule-yearly': function(done) {
        var rrule = new icalendar.RRule({FREQ: 'YEARLY', BYMONTH: 3, BYDAY: '1SU'});
        var start = new Date(1970,2,8,2,0,0);
        assert.equal((new Date(2011,2,6,2,0,0)).toString(),
            rrule.nextOccurs(start, new Date(2011,1,4)).toString());

        // Try a value that is past the matching date in that year
        assert.equal((new Date(2012,2,4,2,0,0)).toString(),
            rrule.nextOccurs(start, new Date(2011,3,7)).toString());

        // A date that already matches should return the next date
        assert.equal((new Date(2012,2,4,2,0,0)).toString(),
            rrule.nextOccurs(start, new Date(2011,2,6,2,0,0)).toString());

        // Every five years...
        var rrule5 = new icalendar.RRule({FREQ: 'YEARLY', INTERVAL: 5, BYMONTH: 3, BYDAY: '1SU'});
        assert.equal((new Date(2015,2,1,2,0,0)).toString(),
            rrule5.nextOccurs(start, new Date(2011,1,4)).toString());

        done();
    },

    'timezone object': function(done) {
    ...
    'parse torture test': function(done) {
        var cal = icalendar.iCalendar.parse(
